    ...
    try:
        async for message in websocket:
            ...
            if data[:5] == 'Start':
                ...
                print(finishing)
                if finishing == 0:
                    send_tasks = [asyncio.create_task(client.send(json.dumps('Finish'))) for client in connected_clients_server1]
                    await asyncio.wait(send_tasks)
                    finishing += 1
                else:
                    finishing = 0

            elif any(entry['Value'] in ["WIN", "DRAW"] for item in data for entry in item if isinstance(entry, dict)):
                judge_dict = {}
                for item in data:
                    for entry in item:
                        key = entry['Key'].replace('"', '')  # Keyからダブルクオートを除去
                        value = entry['Value']  # Valueを取得
                        judge_dict[key] = value
                send_tasks = [asyncio.create_task(client.send(json.dumps(judge_dict))) for client in connected_clients_server1]
                await asyncio.wait(send_tasks)
                players_list.clear()
                start_list.clear()
                count_list.clear()
