...
# 各クラスに一意のラベルを付与してStratifiedKFoldに渡す
class_labels_for_split = all_training_feature_df["class"].values

filter_size_list = [2, 4, 8, 16, 32, 64]
kernel_size_list = [4, 6, 8, 10, 12]
...
for i in range(len(filter_size_list)):
    for j in range(len(kernel_size_list)):
        score_train_array = []
        score_test_array = []
        for train_index, test_index in kf.split(m_cnn_data, class_labels_for_split):
